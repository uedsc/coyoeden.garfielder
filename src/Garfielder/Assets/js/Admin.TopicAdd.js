Garfielder.M("TopicAdd", {
	init: function (opts) {
		...
		p.initAutoSlug = function () {
			...
			//slug input
			me.ui.slug.$input.focus(function () {
				p.slug = this.value;
			});
	...
	RefreshAttachments: function () {
		me.ui.$tabs.filter("#E-edtbtn_files").trigger("click");
	},
	UpdateLogo: function (logo) {
		Garfielder.Cfg.URL_LOGO = logo;
		$("#topic-logo").attr("src", logo).show();
		$("#topic-cover-tip").hide();
	}
}); 